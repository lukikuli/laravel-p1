...
<body>
    <div class="header">
        ...
        <h4 style="line-height: 0px;">Invoice: #{{ $order->no_faktur }}</h4>
        ...
        <table class="layout display responsive-table">
            ...
            <tbody>
                @php 
                    ...
                    $total = 0;
                @endphp
                @forelse ($order->detailpenjualans as $row)
                <tr>
                    <td>{{ $no++ }}</td>
                    <td>{{ $row->barang->nama }}</td>
                    <td>Rp {{ number_format($row->harga_barang) }}</td>
                    <td>{{ $row->jmlh }} Item</td>
                    <td>Rp {{ number_format($row->harga_barang * $row->jmlh) }}</td>
                </tr>
​
                @php
                    $totalPrice += $row->harga_barang;
                    $totalQty += $row->jmlh;
                    $total += ($row->harga_barang * $row->jmlh);
                @endphp
                @empty
                <tr>
