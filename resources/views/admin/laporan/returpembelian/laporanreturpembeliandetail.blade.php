...
@section('content')
    ...
    <section class="main-section">
        ...
        <div class="content">
            ...
            <h1>Laporan Retur Pembelian By Detail</h1>
            <hr>
            <form action="{{ route('laporanreturpembelian.detail') }}" method="get">
                <div class="row">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control select2" id="faktur" name="faktur">
                                ...
                                {
                                    <option value="{{ $order->no_faktur }}" {{ request()->get('faktur') == $order->no_faktur ? 'selected':'' }}> {{ $order->no_faktur }} ({{ $order->supplier->nama }})</option>
                                }
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <a href="{{ route('laporanreturpembelian.detail') }}" class="btn btn-md btn-danger">Reset</a>
                        </div>
                    </div>
                </div>
            </form>
            ...
            <div class="table-responsive">
                <table class="table table-bordered" id="MyTableReport">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><strong>{{ $retur->returpembelian->no_retur }}</strong></td>
                            <td>{{ date('d-m-Y', strtotime($retur->returpembelian->tgl_retur)) }}</td>
                            <td>{{ $retur->returpembelian->pembelian->supplier->nama }} ({{ $retur->returpembelian->pembelian->supplier->kota }}) - {{ $retur->returpembelian->pembelian->supplier->telp_1 }}</td>
...
$(document).ready(function() {
  ...
  $('#MyTableReport').DataTable({
        ...
        buttons: [
            {
                extend: 'print',
                title: 'Laporan RETUR PEMBELIAN | Dragon Koi',
            ...
            {
                extend: 'excel',
                title: 'LAPORAN RETUR PEMBELIAN BY DETAIL',
                ...
                filename: function() {
                    ...
                    return  'Laporan Retur Pembelian By Detail - ' + date_edition;
                }
            }
        ],
        paging: true,
